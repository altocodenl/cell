...
Wait is creating/setting listeners. In gotoB, we have both responder functions (rfuns) and view functions (vfuns), but they are both listeners. Why did I have this double mechanism, compared with the single one I'll propose in TODIS/cell? TODIS/cell has an implicit "change" verb: when a location changes, we activate any sequences waiting for that location. So, vfuns are the same as TODIS/cell waits. Why do we need other verbs, as we have in rfuns? You could just see the verbs as locations, simply. In gotoB, they are not locations on the store, but rather on the responder state. But they could well be folded onto the dataspace. Would that be practical? Well, you could simply make a call (say, ajax), those are normal calls. I think that in gotoB I used rfuns where I would have normally used function calls, so that their traces would be on the eventlog. The places where I definitely needed events were where I was listening to a change in the store. I cannot think of a single rfun that didn't depend on the store that actually would need to be expressed as a wait in cell. But I'll be aware to see if it emerges.
